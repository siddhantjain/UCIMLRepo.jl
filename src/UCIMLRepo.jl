...
function download_http(url::String)
...
#converting received data into ASCII format and returning the value
    rcvd_data = String(request.body)
end

#This function converts the input string into a dataframe
function parse_rcvd_data(rcvd_data::String)
    rows = String[]
    rows = split(rcvd_data,'\n',keep = false)
    df = DataFrame()

    for i = 1:size(rows)[1]
        df[i] = split(rows[i],',',keep =true)
    end
    df
end
...
function parse_rcvd_info(rcvd_info::String)
    rows = String[]
...
function ucirepodata(DsetNameOrUrl::String, DorU::Bool=true)
    if(DorU)
...
function ucirepoinfo(DsetNameOrUrl::String, DorU::Bool=true)
    if(DorU)
	#creating the url to be used for download
